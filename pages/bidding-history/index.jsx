...
function Bidding({ setDates, data, gali }) {
    ...
    return biddingRecords.map((item, index) => (
      <div key={index} className="border-2 border-yellow-400 rounded-lg p-4 mb-4 shadow-lg bg-gray-900 text-white">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div><strong className="text-yellow-300">Amount:</strong> {item.points}</div>
          <div><strong className="text-yellow-300">Game Type:</strong> {item.pana}</div>
          <div><strong className="text-yellow-300">Date and Time:</strong> {item.bid_date}</div>
        </div>
      </div>
    ));
  };
  ...
  return (
    <div className="flex flex-col items-center justify-start min-h-screen bg-gradient-to-b from-gray-900 to-black text-white p-6">
      ...
      <div className="bg-gray-800 p-6 rounded-xl shadow-lg w-full max-w-3xl mt-20">
        <h3 className="text-3xl font-bold text-orange-500 text-center mb-4">Bidding History</h3>
